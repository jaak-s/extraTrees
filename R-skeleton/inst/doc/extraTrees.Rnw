...
A simple usage example is given in Figure \ref{fig:basic.example}. Try changing the value of numRandomCuts to 5 and see how the performance changes. For some data also the value of mtry (the number of chosen features) should be increased.
\begin{figure}
\begin{verbatim}
library(extraTrees)
## data:
...
\end{verbatim}
\caption{Example of using \texttt{extraTrees} with 5 cuts.}
...
For classification the cut that minimizes Gini impurity index is used and for regression the variance is minimized at each node.
